...
@onready var chase_hitbox: CollisionShape2D = $ChaseArea/ChaseHitbox
...
func switch_to_run() :
	if state != EnemyState.Dead and not dead :
...
func handle_animation():
	...
	elif state == EnemyState.Attack and is_attacking and not is_taking_knockback:
		if animated_sprite.animation != "Attack":
			animated_sprite.play("Attack")
	elif state in [EnemyState.Run, EnemyState.Chase]:
...
func take_damage(damage_amount : int , attacker_position : Vector2 , knockback_force : int , upback_force) :
		...
		if animated_sprite.animation == "Attack" :
			if animated_sprite.frame == 5 :
				if player_in_attack_range and player.has_method("take_damage") :
					player.take_damage(attacking_damage , global_position , knockback_force )
